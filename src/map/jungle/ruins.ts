...
import { later, type Sculpted } from './_ruinOfferings';
 ...
 *
 * Solid voxels on the land (1 m blocks, 0.5 m and finer for carvings,
 * plinths and tables), one mesh per block family for each site (sites a
 * stone's throw apart share one), so the sites out of view are not drawn
 * and cast shadows only while those can be seen (cull.ts); the explorer
 * walks among them and through the root gate. Incense smoke, candle flames and
 ...
 *
 * The shrines' statues, stupa and offerings are sculpted (sacred/: smooth
 * sandstone, gilt, brass and wax, not blocks) in one `SacredSet`, on voxel
 * plinths and tables; hidden blocks (never drawn) make the statues and the
 * stupa solid, so the explorer walks round them, not through.
 */

type SiteBuilder = (fr: SiteFrame, lights: ShrineLights, sacred: Sculpted) => void;
...
export function buildJungleRuins(ctx: MapContext): MapPart {
  ...
  for (const site of JUNGLE_SITES) {
    ...
    const solid = new SiteFrame(ctx.field, site);
    make(fr, lights, { set: sacred, solid });
    fr.emit(b);
    solid.emit(hidden);
    built.push(site);
  ...
  let glow: ShrineGlow | null = null;
  later(() => object.add((glow = new ShrineGlow(lights)).object));
  for (const part of splitByPlace(b, built, JOIN).parts) object.add(buildVoxelMesh(part, { quality: 'medium', name: 'jungle' }));
  // The sculpted bodies' hidden blocks: in the walk map (roam/walkmap.ts), never drawn, never picked.
  const solid = buildVoxelMesh(hidden, { quality: 'low', name: 'jungle-solid', castShadow: false, receiveShadow: false });
  solid.visible = false;
  solid.traverse((o) => (o.raycast = () => {}));
  object.add(solid, sacred.object);
