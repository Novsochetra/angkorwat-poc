...
const CLOSEST = 1.6;
/** Seconds the auto-follow waits after the player turned the camera. */
const HANDS_OFF = 1.6;
 ...
 * - Nothing solid comes between it and him: when a wall, a tree trunk or the
 *   land is in the way it pulls in at once (and rises to look over a wall
 *   close behind him), then eases back out.
 * - It never goes into the ground or the water.
 * - While he moves it swings round behind his heading by itself (`follow`),
 *   a moment after the player last turned it, and not while he walks
 *   towards it.
...
export class OrbitFollowCam implements FollowCam {
  ...
  update(dt: number, world: RoamWorld): void {
    ...
    if (this.follow > 0 && this.idle > HANDS_OFF && this.speed > 0.5) {
      ...
      const k = damp(this.follow * 2.5 * Math.min(1, this.speed / 4) * facing, dt);
      this.yaw += diff * k;
    }
    ...
    this.reach = room < this.reach ? room : this.reach + (room - this.reach) * damp(2.2, dt);
  ...
  private free(world: RoamWorld, yaw: number, pitch: number, dist: number): number {
    ...
    for (const k of [1, -1]) t = Math.min(t, world.clearance!(o.x, o.y, o.z, p.x + sx * k, p.y, p.z + sz * k));
    return t;
  }

  /** Lowest the camera may be at its spot: over the ground (or the floor under a roof) and the water. */
  private floorUnder(world: RoamWorld, pos: Vector3): number {
    ...
    const ground = world.standAt ? world.standAt(pos.x, pos.z, pos.y, 0.9, 0) : world.groundAt(pos.x, pos.z);
