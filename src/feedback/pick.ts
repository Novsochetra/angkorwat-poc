...
/** One thing the user clicked while reporting, described for the report. */
export interface Pick {
  kind: 'block' | 'mesh' | 'collider' | 'sky';
...
function meshPick(hit: Intersection, colliders?: ColliderWorld): Pick {
  ...
  let trace: SourceTrace | undefined;
  let block: Matrix4 | undefined;
  ...
  if (inst.isInstancedMesh && hit.instanceId !== undefined) {
    ...
    block = _local.clone().premultiply(inst.matrixWorld);
    ...
    trace = (inst.userData.voxelSources as (SourceTrace | undefined)[] | undefined)?.[id];
    ...
    facts.push(['Instance', `#${id} of \`${inst.name}\``]);
  } else {
    // Plain meshes (ground, water) carry their trace in userData.source.
    trace = ancestor(mesh, (a) => !!a.userData.source)?.userData.source as SourceTrace | undefined;
    const material = mesh.material as Material;
    facts.push(['Mesh', `\`${label}\` · material \`${material.name || material.type}\``]);
  }
  const p = hit.point;
  return { kind: block ? 'block' : 'mesh', point: p.clone(), label, facts, trace, block, colliders: colliders?.boxesAt(p.x, p.y, p.z) ?? [] };
}
